...
テキストを、決定論的なサイン波の音程列へ変換する TypeScript ライブラリです。
一般的な音声合成のような人の声ではなく、アクセント句・モーラ長・感情・声質を
電子的な「発話らしい信号」として表現します。人間ではないキャラクターのための「声」です。

- 同じテキスト・speaker・emotion からは常に同じ PCM / WAV を生成
- kuromoji.js による日本語の読み・品詞解析と、未ロード時の軽量フォールバック
- 5 speaker × 7 emotion を独立して組み合わせ可能
- 44.1 kHz / mono / 16-bit WAV と、字幕・口パク用タイミングを出力
- Node.js とブラウザの双方で利用可能
